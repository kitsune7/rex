...
class WakeWordRecorder:
    ...
    def record_sample(self, duration=2.0, wake_word="rex", sample_type="positive"):
        ...
        return self.save_audio(audio_data, wake_word, sample_type)

    def save_audio(self, audio_data, wake_word, sample_type):
        if audio_data:
            ...
            filename = f"{wake_word}_{sample_type}_{timestamp}.wav"
            filepath = os.path.join(self.output_dir, wake_word, sample_type, filename)
    ...
    def batch_record(self, wake_word, num_samples=20, sample_type="positive"):
        ...
        if sample_type == "positive":
            print(f'\nInstructions: Say "{wake_word}" clearly when prompted')
            print("Tips:")
            print("- Vary your tone slightly between recordings")
            print("- Speak at normal volume")
            print("- Try different distances (1-6 feet from mic)")
        else:
            print("\nInstructions: Say random phrases WITHOUT the wake word")
            print("Tips:")
            print("- Include some phrases that have nothing to do with the wake word")
            print("- Use some phrases that are close to but different than the wake word")
        ...
        with stream:
            for i in range(num_samples):
                ...
                self.record_sample(
                    duration=2.0 if sample_type == "positive" else 3.0,
                    wake_word=wake_word,
                    sample_type=sample_type,
                )

                if i < num_samples - 1:
                    print("Take a short break... (2 seconds)")
                    time.sleep(2)

        print(f'\n Completed recording {num_samples} {sample_type} samples for "{wake_word}"!')
